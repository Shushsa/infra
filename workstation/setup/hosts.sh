...
if [ ! -f "$KIRA_SETUP_CERTS" ] ; then
    ...
    CDHelper text lineswap --insert="127.0.0.1 localhost $KIRA_REGISTRY_NAME" --contains-not="$KIRA_REGISTRY_NAME" --prefix="127.0.0.1" --path=$HOSTS_PATH --append-if-found-not=True --silent=$SILENT_MODE
